...
export class RecordingStatus {
  ...
  stop() {
    this.recordingStopped();
    // this.item.command = 'screenify.record';
    this.item.text = '$(debug-start) Screenify';
    this.counting = false;
  }

  stopping() {
    this.recordingStopped();
    this.item.text = 'Screenify stopping...';
    this.counting = false;
  }
  ...
  start() {
    this.item.command = 'screenify.stop';
    this.item.text = '$(debug-stop) Screenify';
  ...
  async countDown(seconds?: number) {
    ...
    this.counting = false;
    this.item.text = 'Screenify starting';
  }
}