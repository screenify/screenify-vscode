    ...
    const node_copy = require("copy-paste")
    const {
      Readable
    } = require('stream');
    ...
    function activate(context) {
       ...
       * @return {Promise} 
       */
      async function copySerializedBlobToClipboard(serializeBlob, isUpload) {
        ...
        myReadableStreamBuffer.put(Buffer.from(bytes).toString());
        // const bufferStream = new stream.PassThrough();
        // serializeBlob.stream()
        // bufferStream.end(Buffer.from(bytes));
        await node_copy.copy(
          myReadableStreamBuffer, (err, result) => {
            if (err) throw err
