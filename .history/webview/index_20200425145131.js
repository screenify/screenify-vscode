      ...
      (function () {
        ...
        function ReactToMouseMove(e) {
          ...
          if (currentTool === 'brush' && dragging && usingBrush) {
            if (loc.x > 0 && loc.x < canvasWidth - 1 && loc.y > 0 && loc.y < canvasHeight - 1) {
              ctx.lineTo(loc.x, loc.y);
              ctx.stroke();
              AddBrushPoint(loc.x, loc.y, mouseDown = true, brushColor = strokeColor, brushSize = line_Width, mode = "draw", tool = currentTool);
            } else if (loc.x < 0 || loc.x > canvasWidth - 1 || loc.y < 0 || loc.y > canvasHeight - 1) {
              AddBrushPoint(loc.x, loc.y,
                mouseDown = false, brushColor = fillColor,
                brushSize = line_Width, mode = "end");
              // console.log("outside")
              // alert("outside")
              ctx.stroke();
              dragging = false
